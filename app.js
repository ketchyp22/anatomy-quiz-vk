...
function initVK() {
 ...
 if (isVKApp && typeof VKIntegration !== 'undefined') {
   console.log('Запуск внутри VK, инициализация VK Bridge...');
   VKIntegration.init(
     // Успешная инициализация
     (data) => {
       console.log('VK интеграция инициализирована', data);
       // Показываем VK кнопки, если они скрыты
       const vkButtons = document.querySelector('.vk-integration-buttons');
       if (vkButtons) vkButtons.style.display = 'block';
     },
   ...
   console.log('Запуск вне VK, интеграция не требуется');
 }
}
 ...
 if (shareButton) {
   shareButton.onclick = () => {
     ...
     VKIntegration.shareResults(shareMessage, [], (success) => {
       if (success) {
         showNotification('Результат успешно опубликован!');
       } else {
         showNotification('Не удалось опубликовать результат');
       }
     });
   };
 }
 ...
 if (inviteButton) {
   inviteButton.onclick = () => {
     VKIntegration.inviteFriends((success) => {
       if (success) {
         showNotification('Приглашения отправлены!');
       } else {
         showNotification('Не удалось отправить приглашения');
       }
     });
   };
 }
