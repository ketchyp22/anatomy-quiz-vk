...
function selectRandomQuestions() {
    console.log("Вызвана функция selectRandomQuestions()");
    
    if (typeof questions === 'undefined') {
        console.error('Ошибка: массив questions не определен. Проверьте порядок загрузки скриптов.');
    ...
    const selected = shuffled.slice(0, totalQuestionsToShow);
    
    console.log(`Выбрано ${selected.length} вопросов для квиза`);
    return selected;
}

function startQuiz() {
    console.log("Вызвана функция startQuiz()");
    ...
    console.log(`Получено ${questionsForQuiz.length} вопросов для квиза`);
